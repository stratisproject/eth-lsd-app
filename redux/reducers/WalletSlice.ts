...
import { STORAGE_KEY_DISCONNECT_METAMASK, saveStorage } from "utils/storageUtils";
...
export const walletSlice = createSlice({
  ...
  initialState,
  reducers: {
    setMetaMaskAccount: (state: WalletState, action: PayloadAction<string | undefined>) => {
      state.metaMaskAccount = action.payload;
    },
    setMetaMaskChainId: (state: WalletState, action: PayloadAction<string | undefined>) => {
      state.metaMaskChainId = action.payload;
    },
    setMetaMaskDisconnected: (state: WalletState, action: PayloadAction<boolean>) => {
...
export const { setMetaMaskAccount, setMetaMaskChainId, setMetaMaskDisconnected } = walletSlice.actions;
